...
Simple console application that allows one to apply text filters to a string.

Currently only word filters have been implemented, these will filter out words and replace them as dictated by a provided delegate.

## Limitations
* Numerals (ie 0, 1, 2, ..., 9) are considered as valid characters in a word, (eg `hell5`, `5h3hfm2`, and `3243` are all considered as words).
* All punctuation is parsed as a word seperator, (eg `you're` will be parsed as two words, `you` and `are`).


## Example

The console application will currently filter the below sample text with the following three filters:
...
use   ,'   '   ?' she  considering   own  ( 
 ...
 once considering    world  she     .The        
  , and  dipped  ,            
